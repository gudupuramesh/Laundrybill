/**
 * One-off: translate en.json mobile.* to Bengali via Google Translate (gtx),
 * preserving {{placeholders}}. Writes scripts/mobile-bn.json
 * Run: node scripts/generate-mobile-bn-json.cjs
 */
...
const outPath = path.join(__dirname, "mobile-bn.json");
...
function translate(text) {
  ...
  const url = `https://translate.googleapis.com/translate_a/single?client=gtx&sl=en&tl=bn&dt=t&q=${q}`;
...
(async () => {
  ...
  fs.writeFileSync(outPath, JSON.stringify(mobileBn, null, 2) + "\n", "utf8");
