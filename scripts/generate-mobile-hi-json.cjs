/**
 * One-off: translate en.json mobile.* to Hindi via Google Translate (gtx),
 * preserving {{placeholders}}. Writes scripts/mobile-hi.json
 * Run: node scripts/generate-mobile-hi-json.cjs
 */
...
const outPath = path.join(__dirname, "mobile-hi.json");
...
(async () => {
  ...
  fs.writeFileSync(outPath, JSON.stringify(mobileHi, null, 2) + "\n", "utf8");
