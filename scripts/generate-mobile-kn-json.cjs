/**
 * One-off: translate en.json mobile.* to Kannada. Writes scripts/mobile-kn.json
 * Run: node scripts/generate-mobile-kn-json.cjs
 */
...
const outPath = path.join(__dirname, "mobile-kn.json");
...
function translate(text) {
  ...
  const url = `https://translate.googleapis.com/translate_a/single?client=gtx&sl=en&tl=kn&dt=t&q=${q}`;
...
(async () => {
  ...
  fs.writeFileSync(outPath, JSON.stringify(mobileKn, null, 2) + "\n", "utf8");
