/**
 * One-off: translate en.json mobile.* to Malayalam. Writes scripts/mobile-ml.json
 * Run: node scripts/generate-mobile-ml-json.cjs
 */
...
const outPath = path.join(__dirname, "mobile-ml.json");
...
function translate(text) {
  ...
  const url = `https://translate.googleapis.com/translate_a/single?client=gtx&sl=en&tl=ml&dt=t&q=${q}`;
...
(async () => {
  ...
  fs.writeFileSync(outPath, JSON.stringify(mobileMl, null, 2) + "\n", "utf8");
