/**
 * One-off: translate en.json mobile.* to Tamil via Google Translate (gtx),
 * preserving {{placeholders}}. Writes scripts/mobile-ta.json
 * Run: node scripts/generate-mobile-ta-json.cjs
 */
...
const outPath = path.join(__dirname, "mobile-ta.json");
...
function translate(text) {
  ...
  const url = `https://translate.googleapis.com/translate_a/single?client=gtx&sl=en&tl=ta&dt=t&q=${q}`;
...
(async () => {
  ...
  fs.writeFileSync(outPath, JSON.stringify(mobileTa, null, 2) + "\n", "utf8");
