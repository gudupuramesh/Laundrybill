...
/**
 * Temporary shell for roles whose native screens aren't built yet (manager).
 * Confirms routing works and lets the user sign out.
 */
function PlaceholderShell({ role }: { role: string }) {
  const { agent, shopName, signOutAgent } = useDriverAuth();
  return (
    <View style={[styles.center, { padding: 24 }]}>
      <MaterialIcons name="construction" size={48} color={colors.primary} />
      <Text style={styles.phTitle}>{role}</Text>
      <Text style={styles.phSub}>
        Hi {agent?.name || ''} — your {role} workspace is coming soon.
      </Text>
      {shopName ? <Text style={styles.phShop}>{shopName}</Text> : null}
      <TouchableOpacity style={styles.phBtn} activeOpacity={0.85} onPress={signOutAgent}>
        <Text style={styles.phBtnText}>Sign out</Text>
...
function Root() {
  ...
  if (memberType === 'staff') {
    // Staff + Manager both use the staff shell for now; manager-only extras land in Phase 4.
    return <StaffShell />;
  }
