...
export const revenueCatWebhook = onRequest(async (req, res) => {
    ...
    if (activeEvents.includes(eventType)) {
        await subRef.set({
            ...
            status: "active",
            provider,
            providerRef: `rc_${productId}`,
            isAutoRenew: eventType !== "CANCELLATION",
            purchaseState: "active",
            ...(expiresAt ? { currentPeriodEnd: expiresAt, endDate: expiresAt } : {}),
            updatedAt: now,
            lastPaymentDate: now,
        ...
        await subRef.set({
            ...
            cancelledAt: now,
            cancelledBy: "user",
            ...(expiresAt ? { activeUntil: expiresAt } : {}),
            updatedAt: now,
        }, { merge: true });
        ...
        await subRef.set({
            status: newStatus,
            isAutoRenew: false,
            ...(newStatus === "expired" ? { expiredAt: now } : { graceEndDate: expiresAt || now }),
            updatedAt: now,
            ...(eventType === "BILLING_ISSUE" ? { lastPurchaseError: "Billing issue detected by RevenueCat" } : {}),
        }, { merge: true });
