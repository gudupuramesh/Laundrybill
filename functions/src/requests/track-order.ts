...
export const trackOrder = onCall(async (request) => {
    const { code, phone } = (request.data || {}) as { code?: string; phone?: string };
    ...
    if (!orderDoc) {
        ...
        throw new HttpsError("not-found", "No order matches that number and phone. Please check and try again.");
    }
    ...
    if (o.assignedAgentId) {
        try {
            ...
            if (staffDoc.exists) agentPhone = (staffDoc.data()!.phone as string) || "";
            if (!agentPhone) {
                const tmDoc = await db.collection("shops").doc(shopId).collection("teamMembers").doc(o.assignedAgentId).get();
                if (tmDoc.exists) agentPhone = (tmDoc.data()!.phone as string) || "";
            }
        } catch { /* ignore */ }
    }

    const timeline = Array.isArray(o.timeline)
        ? o.timeline.map((e: any) => ({ status: e.status, timestamp: tsToMillis(e.timestamp), note: e.note || e.notes || null }))
        : [];
    ...
    return {
        ...
        items,
        total: f.total ?? o.totalAmount ?? 0,
        amountPaid: f.amountPaid ?? o.paidAmount ?? 0,
        balance: f.balance ?? ((f.total || 0) - (f.amountPaid || 0)),
