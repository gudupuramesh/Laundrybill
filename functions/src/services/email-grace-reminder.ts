...
interface GraceReminderEmailData {
    shopName: string;
    planName: string;
    dayOfGrace: number; // 1, 3, 5, or 7
    daysLeft: number;
    graceEndDate: string;
...
export function getGraceReminderTemplate(data: GraceReminderEmailData): string {
    const {
        shopName,
        planName,
        dayOfGrace,
    ...
    <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="background-color: #f4f4f5;">
        <tr><td align="center" style="padding: 40px 20px;">
            <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="600" class="w-full" style="max-width: 600px; background-color: #ffffff; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
                ${getEmailHeader(settings, "Action Required")}
