/**
 * One active mobile session per account.
 *
 ...
 * same account logs in on another phone, that phone overwrites the slot → this one
 * detects the change and signs out. The web uses a separate `web` slot, so a phone
 * + a browser can both be active.
 */
