...
export function HelpButton({ pageId }: { pageId: string }) {
  ...
  // First-time auto-popup: for new shops, show each page's tutorial once.
  useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
        const enabled = await AsyncStorage.getItem('tutorials_autoshow_enabled');
        if (enabled !== '1') return;
        const seenKey = `tutorial_shown_${pageId}`;
        ...
        const hasVideos =
          (Array.isArray(entry?.videos) && entry!.videos!.some((v) => v.url)) || !!entry?.videoUrl;
        if (!hasVideos) return; // nothing to show yet — try again next visit

        await AsyncStorage.setItem(seenKey, '1');
        if (!cancelled) {
          // small delay so the screen settles before the sheet slides up
  ...
  return (
    <>
      ...
      <TutorialVideosSheet visible={open} onClose={() => setOpen(false)} pageId={pageId} />
    </>
  );
}
