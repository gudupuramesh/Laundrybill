  ...
  const itemRows = (order.items || []).map((item: any) => `
    <tr>
      <td style="padding:6px 0;font-size:13px;">
        ...
        <br/><span style="color:#666;font-size:11px;">${escHtml(item.categoryName || '')} · x${item.quantity}${item.pieceCount ? ` (${item.pieceCount} pcs)` : ''} · ${fmt(item.unitPrice)}${escHtml(t('mobile.receiptHtmlItemEa'))}</span>${item.notes ? `<br/><span style="color:#8a6d00;font-size:10px;font-style:italic;">${escHtml(String(item.notes))}</span>` : ''}
      </td>
...
export default function OrderDetailsScreen({
  ...
  const buildTagsHtml = (): string => {
    ...
    const tags = qrTab === 'order'
      ? serviceGroups.map((g) => ({ url: tagImageUrl(orderId, publicId, 320), service: g.name, line2: `${g.qty} ${g.qty === 1 ? 'item' : 'items'}` }))
      : itemTags.map((tag) => ({ url: tagImageUrl(tag.qrData, tag.qrData.replace(orderId, publicId), 320), service: tag.service, line2: `${tag.idxInService}/${tag.serviceTotal}` }));

    const blocks = tags.map((tg) => `
      <div class="tag">
        <div class="shop">${escHtml(shopName)}</div>
        <img class="code" src="${tg.url}" />
  ...
  /** Render the tags to a PDF whose pages are exactly 50mm × tagHeightMm (in PDF points). */
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={[styles.scrollContent, { paddingBottom: 30 + insets.bottom }]} showsVerticalScrollIndicator={false}>
                ...
                <View key={item.id || idx}>
                  <View style={[styles.serviceItem, deliverMode && draft > 0 ? { backgroundColor: '#EBF2FF', borderRadius: 8 } : null]}>
                    <View style={{ flex: 1 }}>
                      ...
                      {item.notes ? <Text style={[styles.itemMeta, { fontStyle: 'italic', color: colors.warning }]}>✎ {item.notes}</Text> : null}
        ...
        {/* ─── Delete order — permanent, owner only (this app is the owner's) ─────────────── */}
        <TouchableOpacity style={[styles.cancelOrderBtn, { marginTop: 10 }]} onPress={confirmDeleteOrder} disabled={deleting}>
          {deleting
      ...
      <Modal visible={qrModal} transparent animationType="fade" onRequestClose={() => setQrModal(false)}>
        <View style={{ flex: 1 }}>
          ...
          <View style={[styles.modalSheet, { paddingBottom: insets.bottom + 16, maxHeight: '90%' }]}>
            ...
            {/* Code style: QR or Barcode (remembered per shop) */}
            <View style={styles.qrTabRow}>
